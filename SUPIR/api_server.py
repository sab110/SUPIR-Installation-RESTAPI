...
#     """Process a single image upscaling job"""
#     global model  # Declare model as global to avoid scoping issues
    
#     try:
#         update_job_status(job_id, "processing", 0.1, "Initializing...")
        
#         # Load required models
#         if settings.apply_supir:
#             try:
#                 load_supir_model(settings.model, settings.checkpoint_type)
#                 update_job_status(job_id, "processing", 0.2, "SUPIR model loaded")
#             except Exception as e:
#                 printt(f"SUPIR model loading failed: {e}")
#                 update_job_status(job_id, "processing", 0.2, "SUPIR model failed to load, using fallback upscaling")
#                 # Don't fail completely, just disable SUPIR
#                 settings.apply_supir = False
        
#         if settings.apply_llava:
#             load_llava_agent()
#             if llava_agent is not None:
#                 update_job_status(job_id, "processing", 0.3, "LLaVA agent loaded")
#             else:
#                 update_job_status(job_id, "processing", 0.3, "LLaVA agent failed to load, skipping captioning")
#                 settings.apply_llava = False
        
#         if settings.face_restoration:
#             load_face_helper()
#             if face_helper is not None:
#                 update_job_status(job_id, "processing", 0.4, "Face restoration loaded")
#             else:
#                 update_job_status(job_id, "processing", 0.4, "Face restoration failed to load, skipping face processing")
#                 settings.face_restoration = False
        
#         # Load and preprocess image
#         update_job_status(job_id, "processing", 0.5, "Loading image...")
        
#         with Image.open(image_path) as img:
#             img_array = np.array(img.convert('RGB'))
        
#         # Create MediaData object
#         media_data = MediaData(media_path=image_path, media_type="image")
#         media_data.media_data = img_array
        
#         # Apply LLaVA captioning if enabled
#         if settings.apply_llava and llava_agent:
#             update_job_status(job_id, "processing", 0.6, "Generating caption...")
#             try:
#                 # Convert numpy array to PIL Image for LLaVA
#                 pil_image = Image.fromarray(img_array.astype(np.uint8))
#                 caption = llava_agent.gen_image_caption(
#                     imgs=[pil_image], 
#                     temperature=0.2, 
#                     top_p=0.7, 
#                     num_beams=1,
#                     qs="Describe this image and its style in a very detailed manner."
#                 )
#                 media_data.caption = caption[0] if caption and len(caption) > 0 else settings.prompt
#             except Exception as e:
#                 printt(f"LLaVA captioning failed: {e}")
#                 media_data.caption = settings.prompt
#         else:
#             media_data.caption = settings.prompt
        
#         # Apply SUPIR upscaling if enabled and model is loaded
#         if settings.apply_supir and model is not None:
#             update_job_status(job_id, "processing", 0.7, "Upscaling image with SUPIR...")
            
#             # Prepare image for SUPIR
#             img_data = HWC3(img_array)
#             img_data = upscale_image(img_data, settings.upscale_size, unit_resolution=32, min_size=1024)
            
#             # Convert to tensor
#             lq = np.array(img_data)
#             lq = lq / 255 * 2 - 1
#             lq = torch.tensor(lq, dtype=torch.float32).permute(2, 0, 1).unsqueeze(0).to(SUPIR_device)[:, :3, :, :]
            
#             # Process with SUPIR
#             try:
#                 # Ensure consistent data types and devices to avoid mismatches
#                 # The key issue is that ae_dtype, model dtype, and device must be consistent
#                 target_dtype = torch.float16  # Use fp16 consistently
#                 target_device = SUPIR_device  # Ensure consistent device
                
#                 # Convert the autoencoder to consistent dtype and device
#                 if hasattr(model, 'first_stage_model'):
#                     model.first_stage_model = model.first_stage_model.to(dtype=target_dtype, device=target_device)
#                     # Also update the ae_dtype attribute
#                     model.ae_dtype = target_dtype
                
#                 # Convert the main model to consistent dtype and device
#                 if hasattr(model, 'model'):
#                     model.model = model.model.to(dtype=target_dtype, device=target_device)
#                     model.model.dtype = target_dtype
                
#                 # Ensure other model components are on the correct device
#                 model.to(device=target_device)
                
#                 # Convert input tensor to the correct dtype and device
#                 lq = lq.to(dtype=target_dtype, device=target_device)
                
#                 # Run SUPIR sampling
#                 samples = model.batchify_sample(
#                     lq, [media_data.caption],
#                     num_steps=settings.edm_steps,
#                     restoration_scale=settings.s_stage2,
#                     s_churn=settings.s_churn,
#                     s_noise=settings.s_noise,
#                     cfg_scale=settings.s_cfg,
#                     seed=settings.seed,
#                     sampler_cls="sgm.modules.diffusionmodules.sampling.RestoreEDMSampler",
#                     color_fix_type=settings.color_fix_type,
#                     use_linear_cfg=settings.linear_cfg,
#                     use_linear_control_scale=settings.linear_s_stage2
#                 )
...
#                     result_array = x_samples[0]
#                     media_data.outputs = [result_array]
#                 else:
#                     raise Exception("SUPIR processing returned None")
                    
#             except Exception as e:
#                 printt(f"SUPIR processing failed: {e}")
#                 # Fall back to basic upscaling
#                 update_job_status(job_id, "processing", 0.8, "SUPIR processing failed, using fallback upscaling...")
#                 img_data = HWC3(img_array)
#                 result_array = upscale_image(img_data, settings.upscale_size)
#                 media_data.outputs = [result_array]
#         else:
#             # If SUPIR is disabled or failed, just upscale with basic interpolation
#             update_job_status(job_id, "processing", 0.7, "Using basic upscaling...")
#             img_data = HWC3(img_array)
#             result_array = upscale_image(img_data, settings.upscale_size)
#             media_data.outputs = [result_array]
        
#         # Save results
#         update_job_status(job_id, "processing", 0.9, "Saving results...")
        
#         output_files = []
#         timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
#         base_filename = f"{job_id}_{timestamp}"
        
#         for i, output_array in enumerate(media_data.outputs):
#             output_filename = f"{base_filename}_{i:03d}.png"
...
#         # Save caption if requested
#         if settings.save_captions and media_data.caption:
#             caption_filename = f"{base_filename}_caption.txt"
#             caption_path = OUTPUT_FOLDER / caption_filename
#             with open(caption_path, 'w', encoding='utf-8') as f:
#                 f.write(media_data.caption)
        
#         # Update job status to completed
#         update_job_status(job_id, "completed", 1.0, "Processing completed successfully", result_files=output_files)
